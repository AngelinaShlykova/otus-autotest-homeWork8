...
Что реализовано:
- REST проект для API Wishlist
- Запрос на авторизацию (POST /api/auth/login)
- TestSuite для тестирования

Для запуска:
1. Открыть SOAP UI
2. File - Open Project - выбери `Wishlist_REST_Project.xml`
3. Запустить запрос Login для получения токена
